...
const streak = useMemo(() => {
 ...
 for (const q of sortedQuestions) {
     ...
     if (ans?.isCorrect) { cur++; max = Math.max(max, cur); }
     else cur = 0;
 }
 return max;
...
>
 ...
 <div>
    ...
    <p className="text-xl font-semibold text-muted-foreground mt-1">{game.topic}</p>
     </div>
     <div className="flex items-center justify-center gap-2">
     <Badge variant="outline" className={`uppercase${DIFFICULTY_COLORS[game.difficulty] ?? ""}`}>
       {game.difficulty}
      </Badge>
      ...
      <Card className="border-2 border-accent/40 bg-accent/5">
       <CardContent className="p-5">
        ...
        <div className="ml-auto text-right">
         <p className="text-4xl font-black tabular-nums text-accent">{myScore}</p>
         <p className="text-xs uppercase tracking-widest text-muted-foreground">points</p>
        </div>
       </div>
       <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
        { icon: Target, label: "Correct", value: `${myCorrect} / ${totalQ}`, color: "text-secondary" },
         { icon: BarChart3, label: "Accuracy", value: `${accuracy}%`, color: "text-primary"},
         { icon: Zap, label: "Best Streak", value: `${streak}`, color: "text-accent" },
         {
             icon: Star,
             label: "Best Type",
             value: bestType ? `${bestType.accuracy}%` : "—",
        sub: bestType ? QUESTION_TYPE_LABELS[bestType.type] ?? bestType.type :undefined,
             color: "text-secondary",
         },
        ].map((s) => (
        <div key={s.label} className="rounded-lg border border-card-border bg-background/40 p-3 text-center">
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay:0.25 }}>
     <Card className="border-card-border bg-card/60 backdrop-blur">
      ...
      <CardTitle className="flex items-center gap-2 text-sm uppercase tracking-widesttext-muted-foreground">
          <Trophy className="h-4 w-4 text-accent" /> Final Standings
         </CardTitle>
         </CardHeader>
         <CardContent className="p-0">
         {participants.map((p, i) => {
          const isMe = p.userId === userId;
          const pAcc = totalQ > 0 ? Math.round((p.correctCount / totalQ) * 100) : 0;
          return (
        <motion.div
        key={p.userId}
        ...
        <div className="flex-1 min-w-0">
            <p className={`font-semibold truncate ${isMe ? "text-accent" : ""}`}>
            {p.userName}
           {isMe && <span className="text-xs text-muted-foreground ml-1">(you)</span>}
            </p>
            <div className="flex items-center gap-2 mt-0.5">
            <Progress value={pAcc} className="h-1.5 w-20" />
            <span className="text-[11px] text-muted-foreground tabular-nums">
             {p.correctCount}/{totalQ} · {pAcc}%
            </span>
            </div>
        </div>
               {/* Score */}
               <div className="text-right shrink-0">
                <span className={`text-xl font-black tabular-nums ${isMe ? "text-accent" :""}`}>
                 {p.totalScore}
                </span>
              ...
              >
               <div className="px-14 pb-4 space-y-2">
                  ...
                  <>
                   ...
                   <p className="text-[10px] font-semibold uppercase tracking-widesttext-muted-foreground mb-1">Your answer</p>
                  <p className="font-medium leading-snug">{myAns.userAnswer}</p>
                   ...
                   <p className="text-[10px] font-semibold uppercase tracking-widesttext-muted-foreground mb-1">Correct answer</p>
                  ...
                  <div className="flex items-center justify-between text-sm">
                   ...
                   {stat && stat.totalAnswered > 0 && (
                       <div className="flex items-center gap-2 text-muted-foreground">
                        <span>{stat.correctCount}/{stat.totalAnswered} playerscorrect</span>
                        <Progress value={stat.percentCorrect ?? 0} className="w-16 h-1.5"/>
                       </div>
                   )}
                  </div>
                 </>
